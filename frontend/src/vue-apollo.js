...
const httpEndpoint = `http://${window.location.host.split(':')[0]}:4000/graphql`;
...
const defaultOptions = {
  ...
  // You can use `wss` for secure connection (recommended in production)
  // Use `null` to disable subscriptions
  wsEndpoint: `ws://${window.location.host.split(':')[0]}:4000/graphql`,
  // LocalStorage token
  tokenName: AUTH_TOKEN,
  // Enable Automatic Query persisting with Apollo Engine
  persisting: false,
...
console.log(`ws://${window.location.host.split(':')[0]}:4000/graphql`);
